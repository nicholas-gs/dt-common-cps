...
from dt_image_processing_utils.constants import (
    X_OFFSET,
    Y_OFFSET,
    SQUARE_SIZE,
    BOARD_WIDTH,
...
__all__ = [
    'Point',
    'GroundPoint',
    'ImageSpaceResdepPoint',
    'ImageSpaceNormalizedPoint',
...
class Point:
    ...
    def from_message(msg) -> "Point":
        """
        Generates a class instance from a ROS message.
        Expects that the message has attributes ``x`` and ``y``.
        If the message additionally has a ``z`` attribute, it will take it
        as well. Otherwise ``z`` will be set to 0.
        Args:
...
class GroundProjectionGeometry:
    ...
    Note:
        All pixel and image operations in this class assume that the
        pixels and images are *already rectified*. If unrectified pixels or
        images are supplied, the outputs of these operations will be incorrect.
    Args:
    ...
    def estimate_homography(
        ...
        By using an image of the checkerboard calibration pattern taken from the
        robot when placed at the origin marking, detects the corners of the
        squares and computes the best homography matrix from them. The arguments
        of this function determine the dimensions and the position of the
        checkerboard with respect to the robot origin.
        For more information on the called to perform these operations,
        consult the OpenCV reference for
